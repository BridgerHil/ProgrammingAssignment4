...
//  SetGameApp
//
//  Created by Bridger Hildreth on 3/18/22.
//
...
struct Cardify: AnimatableModifier {
    ...
    init(isMatched: Bool) {
        rotation = isMatched ? 90 : 0
    }
    init(isMisMatched: Bool){
        rotation = isMisMatched ? 50 : 0
    }
...
extension View {
    ...
    func cardifyMis(isMisMatched: Bool) -> some View {
        self.modifier(Cardify(isMisMatched: isMisMatched))
    }
}
